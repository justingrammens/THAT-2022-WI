...
* Created and Edge Impulse account
* Verified data is being sent and received from your hardware to the cloud


# Example

In this exmaple we will use the mirophone on the Arduion Nano BLE Sense 33 development board to learn how audio classification works and the intelligence that go into edge processing devices to response to "OK Google" and "Hey Siri"

We will be loosely folling this example:
...
Once the projectis setup, let's work on getting data!

1. Seelct "Devices" link on the left to make sure your device is connected.
2. If the daeomn on your board is running you should see the board shows up. 
    ...
    3. If you have not setup your daeon, please look [here](https://github.com/justingrammens/THAT-2022-WI/tree/main/EdgeImpulse/Nano-33-Sense#user-content-edge-impuse-cli) to go through the toolchain setup.
3. Select the "Data Aqusion" link on the left
...
5. Once you have recorded a sample
    1. Click on the "..." for hte sammple audio and select "split sample"
    2. Select the pieces of the audio you wish to have sampled
    3. Select "Split" and mutliple samples will be created in the collected data list
...
2. The goal of this project is build a system that will then be trained well enough to be able to capture the differnet classes of sounds that are heard.

Trying to capture enough data by ourselves to distingush the differences will take a lot of time. Google Speech commands dataset to the rescue! Following the steps on [Keyword Spotting](https://docs.edgeimpulse.com/docs/pre-built-datasets/keyword-spotting) from the Edge Impulse documentation
...
**NOTE**: It might be obvious to some, but when you are trying to classfy audio into a "yes" or "no", if those are the only classes you have loaded, other random noices and samples will be picked up and put into one of those of those two buckets. To effectivey do this classificaiton you actually need 4 classes of audio: "yes", "no", "unkown - i.e. word that are NOT yes or no" and "noise - which are NOT any words."
...
You can pull images of hte spectrogram and details from the [documention on Edge Impulse](https://docs.edgeimpulse.com/docs/tutorials/responding-to-your-voice#5.-configure-the-mfcc-block)
...
Try changing the defaiult of "13" to "23" and regenerate see what happens!
...
1. Click onf *MFCC* on the left sidebwar
...
Once you have everyitng set click "Start training".

As you look at the output, does the format of this look familiar? If you worked on the first exercise during the workshop, you should notice that the feedback during trainin output looks nearly identical to the training he we did in real-time in the [Hellow ML World](https://github.com/justingrammens/machine_learning/blob/master/tensorflow/lite/micro/examples/hello_world/train/train_hello_world_model.ipynb) example I created.

# Test ouf Model

Now that we have the model built we can test it! Edge Impulse by default put aside 20% of our data for the test data set. This is data that the model has not seen and we can use this to valiadate whether the model is working on unseen data. Perform the following steps:

* Select *Model Testing* in the left menu
* Select the *Classify All* button
* Wait for the system to complete. Testing will typcially take a few minutes.

Once you are happy with the model performance, we are ready for the final step! Deploying to device.

# Deploy to Device

The final step in thie exercise, is deploying this model to the Arduino Nano 33 BLE Sense. Edge Impulse has a method to do a one click deploy to your device using the *Deployment* option from the web. To accomplish this:

1. Select *Deployment* from the left menu
2. Under the *Build Frmware* section.
...
To Run on device you will want to flash with hte binary you downloaded.
...
4. When complete, you should see something simlar to
